...
> **v0.1.0-pre / Phase 6 support flow online.** The service now supports independent chat-provider and embedding-provider selection, a real support orchestration path built from StateGraph triage + RAG lookup + native tool calling, and the first live transport surface. Storage, hard caps, prompt-injection guardrails, and compose/demo assets still land in later Phase 6 plans.
...
- `internal/app` owns model construction, embedding bootstrap, seeded knowledge-base setup, wrapped agent construction, transport mux wiring, and `http.Server` startup/shutdown.
...
- `internal/supportflow` owns typed support triage, refund-policy lookup, and human handoff routing.
